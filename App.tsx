...
function App() {
  ...
  const checkRestriction = (): boolean => {
    if (currentUser?.status === "on_hold") {
      alert(
        "Action Restricted: Your account is currently ON HOLD. Please contact support.",
      );
      return false;
    }
    return true;
  ...
  const handleTransaction = (tx: Transaction) => {
    ...
    if (
      ...
      currentUser.fee_percentage &&
      currentUser.fee_percentage > 0 &&
      tx.type !== "receive"
    ) {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case Page.AI_ADVISOR:
        return <AIAssistant assets={assets} />;
      ...
      case Page.ADMIN_USERS:
        return (
          <AdminUsers
            ...
            assets={assets}
            onUpdateUserAsset={(uid, aid, amt) => {
              const userAssets = authService.getUserAssets(uid);
              const updated = userAssets.map((a) =>
                a.id === aid ? { ...a, balance: amt, value: amt * a.price } : a,
              );
      ...
      case Page.PROFILE:
        return <Profile user={currentUser!} onSubmitKYC={handleKycSubmit} />;
      ...
      case Page.CONVERT:
        return (
          <Convert
            assets={assets}
            marketPrices={marketPrices}
            feePercentage={currentUser?.fee_percentage || 0.5}
            onConvert={(from, to, amt, out, fee) => {
              if (!checkRestriction()) return;
              const fromAsset = assets.find((a) => a.symbol === from);
              const toAsset = assets.find((a) => a.symbol === to);
              if (fromAsset)
                handleUpdateAsset(fromAsset.id, fromAsset.balance - amt);
      ...
      case Page.BUY:
        return (
          <BuyCrypto
            marketPrices={marketPrices}
            onBuy={(amount, crypto, fee, method, provider) => {
              if (!checkRestriction()) return;
              ...
              if (asset) {
                ...
                const newAssets = authService.enableWallet(
                  currentUser!.id,
                  crypto.symbol,
                );
                setAssets(newAssets);
                ...
                const targetAsset = freshAssets.find(
                  (a) => a.symbol === crypto.symbol,
                );
                if (targetAsset) {
                  targetAsset.balance = amount;
                  targetAsset.value =
                    amount * (marketPrices[crypto.symbol] || crypto.price);
    ...
    <div className={`dark min-h-screen bg-black`}>
      <div className="flex min-h-screen bg-black text-white relative overflow-hidden">
        ...
        <AddWalletDialog
          ...
          availableCryptos={AVAILABLE_CRYPTOS.filter(
            (c) => !assets.some((a) => a.symbol === c.symbol),
          )}
        ...
        <div className="flex-1 flex flex-col min-h-screen z-10 relative">
          <header className="lg:hidden bg-black/60 backdrop-blur-xl border-b border-[#D4AF37]/20 px-4 py-4 sticky top-0 z-30 flex items-center gap-4">
            ...
            <h1 className="text-lg font-black text-[#D4AF37] tracking-[0.2em]">
              HONOR
            </h1>
          </header>
          <main className="flex-1 p-6 lg:p-12 overflow-y-auto">
            <div className="hidden lg:flex justify-between items-center mb-12">
              <div>
                ...
                <p className="text-[#D4AF37]/50 text-xs mt-3 font-black uppercase tracking-[0.3em]">
                  Honor Secure Gateway | {currentUser.full_name}
                </p>
              </div>
              {currentPage === Page.DASHBOARD && (
                <div className="flex gap-4">
                  <button
                    ...
                    className="px-8 py-4 bg-gradient-to-r from-[#B8860B] to-[#D4AF37] text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-[1.02] transition-all active:scale-95 shadow-lg shadow-[#D4AF37]/20"
                  >
                    Acquire Asset
                  </button>
                </div>
